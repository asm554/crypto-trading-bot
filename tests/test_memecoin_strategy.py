...
def test_momentum_change_pct_requires_half_window_of_history():
    ...
    assert bot._momentum_change_pct("BONK") is None
    # Now span covers >= 30min (half of the 60min window).
    bot._update_history("BONK", 30 * 60.0 + 1, 1.2)
    assert bot._momentum_change_pct("BONK") == pytest.approx(20.0)  # (1.2-1.0)/1.0*100
...
def test_scan_entries_opens_position_on_momentum(monkeypatch, tmp_path):
    ...
    async def scenario():
        ...
        now = 60 * 60.0
        bot.price_history["BONK"] = [[0.0, 1.0], [now - 30 * 60 - 1, 1.05]]

        async def fake_fetch_meme_pairs(_symbols):
            return {"BONK": _pair("BONK", price_usd="1.20")}  # +20% since oldest sample
        ...
        assert bot.portfolio["BONK"]["entry_price"] == pytest.approx(1.20 * 1.015)
...
def test_scan_entries_skips_momentum_outside_band(monkeypatch, tmp_path):
    ...
    async def scenario():
        ...
        now = 60 * 60.0
        bot.price_history["BONK"] = [[0.0, 1.0]]  # +2% -> below entry_change_pct
        bot.price_history["WIF"] = [[0.0, 1.0]]  # +90% -> above entry_max_change_pct

        async def fake_fetch_meme_pairs(_symbols):
            return {
                "BONK": _pair("BONK", price_usd="1.02"),
                "WIF": _pair("WIF", price_usd="1.90"),
            }
...
def test_scan_entries_skips_below_min_liquidity(monkeypatch, tmp_path):
    ...
    async def scenario():
        await paper_db.init_db()
        bot = _bind_bot_to_tmp_storage(
            MemecoinMomentumBot(initial_capital_eur=100.0, min_liquidity_usd=50_000.0, momentum_lookback_min=60.0, paper_mode=True),
            tmp_path,
        )
        bot.symbols = ["BONK"]
        now = 60 * 60.0
        ...
        async def fake_fetch_meme_pairs(_symbols):
            return {"BONK": _pair("BONK", price_usd="1.20", liquidity_usd=1000.0)}
...
def test_manage_positions_exits_via_take_profit(monkeypatch, tmp_path):
    ...
    async def scenario():
        ...
        async def fake_fetch_meme_pairs(_symbols):
            return {"BONK": _pair("BONK", price_usd="1.20")}  # +20% > take-profit 15%
...
def test_scan_entries_respects_max_open_positions(monkeypatch, tmp_path):
    ...
    async def scenario():
        ...
        now = 60 * 60.0
        bot.price_history["BONK"] = [[0.0, 1.0], [now - 30 * 60 - 1, 1.05]]
        bot.price_history["WIF"] = [[0.0, 2.0], [now - 30 * 60 - 1, 2.1]]

        async def fake_fetch_meme_pairs(_symbols):
            return {
                # Beide im Band [8, 60]: BONK +30% mit hoher Liquidität, WIF +25% mit
                # niedrigerer Liquidität -> Score bevorzugt BONK.
                "BONK": _pair("BONK", price_usd="1.30", liquidity_usd=200_000.0),
                "WIF": _pair("WIF", price_usd="2.50", liquidity_usd=60_000.0),
            }
