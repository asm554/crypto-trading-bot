...
from polybot import config, paper_db
import polybot.ultimate_strategy as strategy
from polybot.ultimate_strategy import UltimateBot, advanced_bullish_patterns, analyse_market
...
def _analysis(score=90, regime="uptrend", setup="breakout", atr=1.0):
    return {
        ...
        "support": 95.0, "resistance": 100.0, "close": 102.0,
    }
...
def test_entry_creates_two_paper_tranches(monkeypatch, tmp_path):
    ...
    async def scenario():
        ...
        assert bot.capital_remaining == pytest.approx(87.5)
...
def test_equity_uses_bid_and_both_fees(monkeypatch, tmp_path):
    ...
    async def scenario():
        ...
        expected = 100.0 - 20.0 * config.CRYPTO_TAKER_FEE_RATE
