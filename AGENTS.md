...
- `polybot.main_ultimate` — Adaptive BTC/ETH/SOL market-regime strategy / "Der Ultimative"
...
python -m polybot.main_ultimate
...
**"Der Onchain" (memecoin_strategy.py) is the only bot not on Kraken.** It prices Solana memecoins via the public DexScreener REST API (`fetch_pairs_by_address()`, no wallet/key needed for paper mode) instead of `dca_strategy.py`'s Kraken helpers, and converts USD→EUR itself via a Kraken `EURUSD` ticker lookup. Its universe is hybrid: a curated core of ~12 individually live-verified mint addresses (`SYMBOL_TO_MINT`) plus, if `dynamic_enabled`, tokens discovered from DexScreener's public boost/profile feeds (`discover_dynamic_solana_tokens()`) — those feeds are paid promotion, not an organic signal, so a discovery failure returns `[]` and never blocks the curated core, and dynamic candidates get an extra minimum-pool-age gate (`min_pair_age_hours`) the curated core is exempt from. **Keying is by mint address, not ticker**: Solana is permissionless, so two tokens (curated or dynamic) can share a symbol — a live probe during development found a homoglyph clone of "ai16z" (Greek/Cyrillic look-alike characters) with far more liquidity than the real token, which is exactly the failure mode address-pinning avoids. `portfolio`/`price_history`/`cooldowns` are all keyed by address; `market_question` encodes both as `CHAIN_{symbol}@{address}` (symbol for the dashboard, address for price resolution) — parse with `.partition("@")`, never assume no `@` in the remainder. Entry is a momentum band (`entry_change_pct`..`entry_max_change_pct`, default 8–60 % over `momentum_lookback_min`, computed from the bot's own rolling price history since DexScreener has no OHLC API) plus liquidity, 24h-volume, and h1 buy/sell-ratio gates; candidates are ranked by `change_pct * log10(volume_h24)`. There's no order book, so fills simulate an AMM slippage/price-impact haircut (`slippage_pct`) instead of a real bid/ask spread. Because `battle_report.equity_for()` assumes Kraken pairs, `battle_report.py` has a separate `equity_for_memecoin()` that parses the address out of `market_question` and calls `fetch_pairs_by_address()` instead — if you add data-flow logic to `equity_for()`, check whether it needs mirroring there too.
