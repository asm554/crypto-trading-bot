...
async def main():
    ...
    bot = HodlBot(initial_capital_eur=number("HODL_BUDGET", 100), cash_reserve_eur=number("HODL_CASH_RESERVE_EUR", 20), max_weekly_eur=number("HODL_MAX_WEEKLY_EUR", 20), bear_rate_pct=number("HODL_BEAR_RATE_PCT", 35), overheat_momentum_pct=number("HODL_OVERHEAT_MOMENTUM_PCT", 50), overheat_ema_pct=number("HODL_OVERHEAT_EMA_PCT", 25), paper_mode=os.getenv("HODL_PAPER_MODE", "true").lower() == "true")
