...
async def sync_freqtrade_once(session: aiohttp.ClientSession) -> dict:
    ...
    try:
        ...
        # /api/v1/trades enthält standardmäßig nur abgeschlossene Trades.
        # Offene Dry-Run-Positionen kommen ausschließlich über /status und
        # müssen ebenfalls ins read-only Dashboard gespiegelt werden.
        trade_records = list(trades)
        known_ids = {trade.get("trade_id", trade.get("id")) for trade in trade_records}
        if isinstance(status_rows, list):
            trade_records.extend(
                row for row in status_rows
                if row.get("trade_id", row.get("id")) not in known_ids
            )
        now = time.time()
        ...
        for trade in trade_records:
            ...
            is_closed = closed_at > 0
            close_profit = trade.get("close_profit_abs")
            trade_pnl = close_profit if close_profit is not None else trade.get("profit_abs")
            normalized_trades.append(
                {
                    ...
                    "real_pnl": float(trade_pnl) if is_closed and trade_pnl is not None else None,
                    "unrealized_pnl": float(trade_pnl) if not is_closed and trade_pnl is not None else None,
                }
            )
